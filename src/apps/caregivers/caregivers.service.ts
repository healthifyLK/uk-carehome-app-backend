import {
  ...
  import { EmploymentType, CaregiverStatus } from '../../database/models/caregiver.model';
  import {
    CaregiverRegistrationDto
  } from './dto/caregivers-registration.dto';
...
import { CareReceiverResponseDto } from '../patients/dto/care-receiver-response.dto';
  ...
  export class CaregiversService {
    ...
    async registerCaregiver(
      ...
      if (existingCaregiver) {
        throw new BadRequestException(
          'Caregiver with this email already exists',
        );
      }
  
      // Generate employee ID if not provided
      const employeeId = registrationData.employeeId || await this.generateEmployeeId();
    ...
    async getCaregiverById(caregiverId: string): Promise<CareReceiverResponseDto> {
    ...
    async getAllCaregivers(locationId?: string): Promise<CareReceiverResponseDto[]> {
    ...
    async requestDataDeletion(caregiverId: string, userId: string): Promise<void> {
      const caregiver = await this.caregiverModel.findByPk(caregiverId);
      if (!caregiver) {
        throw new NotFoundException('Caregiver not found');
      }
    ...
    private mapToResponseDto(caregiver: Caregiver): CareReceiverResponseDto {
      return {
        id: caregiver.id,
