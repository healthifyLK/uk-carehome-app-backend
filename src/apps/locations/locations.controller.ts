...
export class LocationsController {
  ...
  async createLocation(
    ...
    return this.locationsService.createLocation(createLocationDto, req.user.id);
  }
  @Get()
  @Roles(UserRole.SUPER_ADMIN, UserRole.ADMIN)
  @ApiOperation({ summary: 'Get all locations with statistics' })
  @ApiQuery({
    name: 'includeStats',
    required: false,
    description:
      'Include statistics (beds, caregivers, care receivers, occupancy rate)',
    type: Boolean,
  })
  @ApiResponse({
    status: 200,
  ...
  async getAllLocations(
    ...
    return this.locationsService.getAllLocations(includeStats ?? true); // Default to true for dashboard
  }
  ...
  @ApiResponse({
    status: 400,
    description: 'Cannot delete location with associated data',
  })
  ...
  @Get('dashboard')
  @Roles(UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.CAREGIVER)
  @ApiOperation({ summary: 'Get locations dashboard data' })
  @ApiResponse({
    status: 200,
    description: 'Dashboard data retrieved successfully',
    type: [LocationResponseDto],
  })
  async getLocationsDashboard(): Promise<LocationResponseDto[]> {
    return this.locationsService.getAllLocations(true);
  }
}
