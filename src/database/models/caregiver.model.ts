...
@Scopes(() => ({
  ...
  withSensitiveData: {
    attributes: {
      include: ['homeAddress', 'emergencyContact', 'rightToWorkDocument'],
    },
  },
}))
@Table({
...
export class Caregiver extends BaseModel<Caregiver> {
  ...
  @BeforeCreate
  @BeforeUpdate
  static async hashPassword(caregiver: Caregiver) {
    if (caregiver.changed('password') && caregiver.password) {
      const saltRounds = 12;
      caregiver.password = await bcrypt.hash(caregiver.password, saltRounds);
    }
  }
